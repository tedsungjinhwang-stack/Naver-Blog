...
4. 인제스트(ingest) 작성한 글 → SEO 자가진단 + 투자권유·과장광고 컴플라이언스 체크
        ↓
5. 이미지(images)  니치·주제별 검색어로 라이선스 안전한 이미지 자동 수집 + 출처 문구 생성
        ↓
6. 출력(output)    Markdown(검토용) + .paste.html(서식 유지 붙여넣기) + .naver.txt(평문)
        ↓
7. 기록(store)     SQLite 실행 로그(중복 주제 회피·성과 추적)
```

핵심 설계 원칙(리서치 반영):
- **투트랙 intent**: `homefeed`(홈판, 기본) vs `search`(검색형) — 분량·제목·발행시간 규칙이 달라집니다.
- **고유성 우선**: 실경험·구체 수치·표/체크리스트를 강제해 "복붙 불가한" 문서로 만듭니다(D.I.A.+ 대응).
- **이미지 라이선스 안전**: NC(비영리)·ND(변경금지)는 항상 제외하고 CC0/PDM/BY만 수집합니다.
  애드포스트 수익화 시 블로그도 상업적 이용이기 때문입니다. BY는 출처 문구가 자동 생성됩니다.
...
네이버는 2020년 5월 **블로그 글쓰기 API를 폐지**했습니다(자동 대량 게재 차단 목적).
따라서 이 파이프라인은 발행을 자동화하지 않습니다. 대신 **서식을 유지한 채 붙여넣는**
방식으로 수작업을 최소화합니다.

1. `output/*.paste.html` 을 브라우저로 엽니다.
2. **제목 복사** → 네이버 에디터 제목칸에 붙여넣기
3. **본문 복사 (서식 유지)** → 본문에 붙여넣기 — 소제목·굵기·표가 그대로 들어갑니다
4. `[사진X]`(수집 완료 파일) · `[이미지N]`(직접 촬영/제작) 자리에 사진을 올리고 표시줄 삭제
5. **태그 복사** → 태그칸에 붙여넣기 → 발행

로그인·발행만 사람이 하므로 계정 리스크가 없습니다.
